...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, act=swish, use_node_features=True, use_extra_node_feature=False):
        ...
        if self.use_node_features:
            ...
            self.emb = Linear(3, hidden_channels)
...
class SphereNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.edge_transform1 = nn.Linear(in_features=hidden_channels, out_features=512)
        self.edge_transform2 = nn.Linear(in_features=512, out_features=1001)
    ...
    def forward(self, batch_data):
        ...
        # print(e)
        # print(torch.isnan(e[1]).sum().item())
        # print(v)
        # print(u)
        # Select only the edges that connects C and H
        # Assume z contains atomic numbers or identifiers
        # where 5 represents carbon and 0 represents hydrogen
        carbon_nodes = (z == 5).nonzero(as_tuple=True)[0]
        hydrogen_nodes = (z == 0).nonzero(as_tuple=True)[0]

        # Create a mask for edges connecting C and H
        ch_edge_mask = [
            (i in carbon_nodes and j in hydrogen_nodes) or 
            (i in hydrogen_nodes and j in carbon_nodes) 
            for i, j in edge_index.t()
        ]

        # Select only the edges that connect C and H
        ch_edge_index = edge_index[:, ch_edge_mask]

        _, e2 = e #### only use e2 feature for now
        # Use only the C-H edges from e2
        ch_e2 = e2[ch_edge_mask]

         # Transform ch_e2 using 2 linear layer
        transformed_ch_e2 = nn.ReLU()(self.edge_transform1(ch_e2))
        transformed_ch_e2 = self.edge_transform2(transformed_ch_e2)
        # Map each edge to its graph in the batch
        # Using the start node (index 0 of each edge) for simplicity
        edge_batch = batch[ch_edge_index[0]]
        # Sum the transformed edge features by batch
        # Assuming you have a tensor 'edge_batch' that indicates the batch index for each edge
        summed_edge_features = scatter(transformed_ch_e2, edge_batch, dim=0, reduce='mean')

        # graph_features = nn.ReLU()(self.edge_transform1(u))
        # graph_features = self.edge_transform2(graph_features)

        return ch_e2, summed_edge_features, v, u
...
def xyz_to_dat(pos, edge_index, num_nodes, use_torsion = False):
    ...
    if use_torsion:
        ...
        ###
        dist_ji = torch.where(dist_ji == 0, torch.full_like(dist_ji, 0.01), dist_ji)
