...
#         z = data.z.long()
        ...
        if self.pred == 'CH':
            # Create a mask for edges connecting C and H
            ch_edge_mask = [
                (i.item() in carbon_nodes and j.item() in hydrogen_nodes) 
                # or (i.item() in hydrogen_nodes and j.item() in carbon_nodes) 
                for i, j in data.edge_index.t()
    ]

            # Select only the edges that connect C and H
            ch_edge_index = data.edge_index[:, ch_edge_mask]

            # Concatenate the node features for each C-H pair
            ch_features = torch.cat([x[ch_edge_index[0]], x[ch_edge_index[1]]], dim=1)
            # print(ch_features.shape)

            # pass ch_features through projection layer before aggregation on graph level
            ch_projection = self.lin_out(ch_features)
            # print(ch_projection.shape)

            # this is the graph level feature
            # use only c-h edge
            edge_batch = batch[ch_edge_index[0]]
            out = scatter(ch_projection, edge_batch, dim=0, reduce=self.agg_method) # default "sum"
        elif self.pred == 'C':
            ...
            for c_node in carbon_nodes:
                ...
                c_to_h_edges.append(connected_hydrogens)
            # Loop through each list of connected hydrogen nodes for each carbon node
            h_features_average = []
            for h_nodes in c_to_h_edges:
                if len(h_nodes) > 0:
                    # Extract features for these hydrogen nodes
                    h_node_features = x[h_nodes]

                    # Calculate the average of these features
                    avg_h_features = torch.mean(h_node_features, dim=0)
                else:
                    # Handle the case where a carbon node is not connected to any hydrogen nodes
                    # Assuming the feature size is known, e.g., feature_size
                    avg_h_features = torch.zeros(self.hidden_channels).to(device)

                h_features_average.append(avg_h_features)
            h_features = torch.stack(h_features_average)
            # add in c_features and take average
            h_features = torch.stack([c_features, h_features], dim=1)
            h_features = torch.mean(h_features, dim=1)
            ...
            energy = scatter(x, batch, dim=0, reduce=self.agg_method) # default "sum"
            # print(energy.shape)
            out = self.lin_out(energy)
...
#     trained_model = trainednet(in_embed_size=3, out_channels=1, \
#                 agg_method='sum', hidden_channels=256, out_hidden=[128], \
#                     num_layers=3, num_output_layers=2)
#     msg = trained_model.load_state_dict(torch.load('data_nmr_alignment/gnn3d_calignment_sum_hiddendim_256_nlayers_3_noutlayers_2_outhidden_128.pt', map_location='cpu'))
#     print(msg)
#     model = ComENet(in_embed_size=3, out_channels=1, agg_method='sum', hidden_channels=256, out_hidden=[128], num_layers=3, num_output_layers=2, pred='C')
    
#     # trained model attributes: feature1 feature2 emb interaction_blocks lins lin_out
