...
class fpt_graph_nmr_data(Dataset):
    def __init__(self, csv_file, fpt_path, graph_path, nmr_path):
        df = pd.read_csv(csv_file)
        self.file_list = df['File_name'].to_list()
        self.nmr_path = nmr_path
        self.graph_path = graph_path
        self.fpt_path = fpt_path
    def __len__(self):
        return len(self.file_list)
    def __getitem__(self, item):
        filename = self.file_list[item].split('.')[0]
        fpt_file = os.path.join(self.fpt_path, filename + '.pkl')
        fpt_data = pickle.load(open(fpt_file, 'rb'))
        ...
        return fpt_data, graph_data, nmr_data, nmr_data_noise, filename
