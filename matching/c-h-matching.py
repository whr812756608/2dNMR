...
if initial_match:
    ...
    model = ComENet(in_embed_size=3, c_out_channels=1, h_out_channels=2, agg_method='sum',\
        hidden_channels=hc, c_out_hidden=chc, h_out_hidden=hhc, num_layers=n, num_output_layers=n_out, dropout=dropout)
    msg = model.load_state_dict(torch.load(
        'gnn3d_nofreeze_solvent4_2dch_dropout%0.2d__sum_hiddendim_%d_nlayers_%d_noutlayers_%d_couthidden_%s_houthidden_%s.pt'\
    %(dropout * 10, hc, n, n_out, ''.join(str(i) for i in chc), ''.join(str(i) for i in hhc)), map_location='cpu'), strict=True) #, map_location='cpu'
    print('loading finetuned model ', msg)

model.eval()

graph_path_2dnmr = '/scratch0/yunruili/2dnmr_30k/graph_3d/'
csv_file_2dnmr = 'nmr_smile_solventclass_filtered4_3dgnn.csv'
nmr_path_2dnmr = '/scratch0/yunruili/2dnmr_30k/csv_30k/'
save_dir = '/scratch0/yunruili/2dnmr_30k/nmr_2dcsv_chmatched_2nd'
